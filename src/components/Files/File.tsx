...
export default function File({
  ...
  return (
    <div
      className={`file_animate mb-2 w-full flex justify-center transition-all duration-500 ${
        isDeleting ? "file_deleting" : ""
      }`}
    >
      <div
        className={`w-full bg-secondaryBg-light dark:bg-secondaryBg-dark flex py-3 rounded-lg hover:bg-stone-300 hover:dark:bg-slate-700 duration-300 transition-all ${
          file.progress ? "file_uploading" : ""
        }`}
      >
        ...
        <div className="ml-3 flex items-center gap-3 mr-4">
          {"progress" in file ? (
            <p className="text-xs flex">
              {file.progress} <span className="ml-1">%</span>
            </p>
          ) : (
            <>
              {auth && (
              ...
              <Link
                href={`${process.env.NEXT_PUBLIC_FILES_SERVER}/api/download/${file.id}?pin=${pin}`}
              >
