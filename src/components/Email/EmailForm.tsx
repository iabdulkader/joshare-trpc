...
export default function EmailForm() {
  const [from, setFrom] = useState<{ value: string; error: string }>({
    value: "",
    error: "",
  });
  const [to, setTo] = useState<{ value: string; error: string }>({
    value: "",
    error: "",
  });
  ...
  const send = (e: FormEvent<HTMLFormElement>) => {
    ...
    if (
      /^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/gi.test(to.value) === false
    ) {
  ...
  return (
    <form className="modal-container" onSubmit={send}>
      <div className="h-full overflow-y-auto flex flex-col items-center">
        ...
        <div className="mb-2">
          <p className="text-xs">
            Attempts Remaining <span className="dot">{emailRemaining}</span>
          </p>
        </div>
